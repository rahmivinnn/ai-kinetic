  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="w-64 bg-slate-900 text-white p-4 hidden md:block">
        <div className="flex items-center justify-center mb-8 py-3 bg-gradient-to-r from-blue-900/40 to-indigo-900/60 rounded-lg shadow-inner">
          {/* Logo removed as requested */}
        </div>

        <nav className="space-y-1">
          {[
            { name: 'Dashboard', icon: <Activity className="h-5 w-5 mr-3" />, href: '/admin-dashboard', active: true },
            { name: 'Pose Detection', icon: <Video className="h-5 w-5 mr-3" />, href: '/pose-detection-v2' },
            { name: 'Analysis', icon: <BarChart2 className="h-5 w-5 mr-3" />, href: '/analysis' },
            { name: 'Calendar', icon: <Calendar className="h-5 w-5 mr-3" />, href: '/calendar' },
            { name: 'Reports', icon: <FileText className="h-5 w-5 mr-3" />, href: '/reports' },
            { name: 'Settings', icon: <Settings className="h-5 w-5 mr-3" />, href: '/settings' },
            { name: 'Profile', icon: <User className="h-5 w-5 mr-3" />, href: '/profile' },
          ].map((item) => (
            <Link
              key={item.name}
              href={item.href}
